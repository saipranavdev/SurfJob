...
class ematch_class :
    ...
    def build_sql_query_stmt (self):
        ...
        for a_pair in cgi_pairs:
            ...
            if ((one_pair[0] == 'specialty') or (one_pair[0] == 'company_name')) \
                    and (one_pair[1] != 'all' and one_pair[1] != 'any'):
                if pair_n > 0:
                    sql_stmt += " AND "
                else:
                    sql_stmt += " WHERE "
                ...
                if one_pair[0] == 'specialty':
                    ...
                    self.askedCompany = one_pair[1]
                    paraList.append(self.askedCompany.lower())
                    matched_companies = []
                    matched_companies = self.find_matched_companies(self.askedCompany.lower())
                    if matched_companies:
                        sql_stmt += " OR lower(company_name) IN (" + \
                                    ','.join(["'{}'".format(company.lower()) for company in matched_companies]) + \
                                    ")"
                        #sql_stmt += " ORDER BY lower(company_name)"
                pair_n += 1

            if (one_pair[0] == 'min_salary') and (one_pair[1] != 'all' and one_pair[1] != 'any'):
                self.askedMinSalary = int(one_pair[1])
            
            if (one_pair[0] == 'field_keyword') and (one_pair[1] != 'all' and one_pair[1] != 'any'):
                self.askedKeyword = one_pair[1]
            if (one_pair[0] == 'location_type'):
                self.askedLocation = one_pair[1]
            if (one_pair[0] == 'job_type'):
                self.askedJobType = one_pair[1]
    ...
    def find_matched_companies(self,searchedcomp):
        comm = []
        uscomparray= ematch_data_struct.company_array
        usrelatedcompanies=ematch_data_struct.relatedCompanies
        if(searchedcomp in uscomparray.keys()):
            comm = usrelatedcompanies[uscomparray[searchedcomp]]
        return comm    
    
    def find_keywords(self):
        if(self.askedKeyword):
            company_dicts=ematch_data_struct.company_dict
            for key, value in company_dicts.items():
                values = value.split(" ")
                # print(values)
                if (len(values)>0):
                    for i in range(len(values)):
                        if self.askedKeyword == values[i]:
                            self.filteredcompanies[key]=1
                            print(values[i])
                            break
                        else:
                            if values[i].find(self.askedKeyword)!=-1:
                                self.filteredcompanies[key]=2


                            # pattern = re.escape(self.askedKeyword) + r'\w*'  # Match the word followed by optional extra characters
                            # if re.search(pattern, self.askedKeyword):
                                # self.filteredcompanies[key]=2
            # if searchkeyword in value:
    ...
    # Check if job type is specified in the search
        if self.askedJobType and self.askedJobType != 'all' and self.askedJobType != 'any':
            jobType = row_[1].lower()  # Assuming job type is at index 9 in the row, adjust if needed
            penalty = self.compute_job_type_penalty(jobType)
            deduction += penalty
        if self.askedCompany and self.askedCompany!='all' and self.askedCompany!='any':
            comp = row_[10].lower()
            if self.askedCompany!=comp :
                dec_rat = (ematch_data_struct.company_array[comp]) + 10
            else :
                dec_rat=0
            deduction+=dec_rat
        ...
        return jobRating
    def keyword_penalty(self,row_):
        if self.askedKeyword and self.askedKeyword!='all' and self.askedKeyword!='any':
            dec=0
            # print("filteredcompanies",self.filteredcompanies)
            if row_[10].lower() in self.filteredcompanies:
                if self.filteredcompanies[row_[10].lower()]==1:
                    return 100
                else:
                    return 91
    ...
    def compute_job_type_penalty(self, jobType):
        # Define penalty based on the queried job type
        queriedJobType = self.askedJobType.lower()
        if jobType == queriedJobType:
    ...
    def process_results (self,con,stmt):
        ...
        for row in cursor:
            ...
            if self.askedKeyword and row[10].lower() in self.filteredcompanies:
                jobRating=self.keyword_penalty(row)
                arr.append(row)
                ratings_arr.append(jobRating)
                tr+=1
                keyword_index = 1
            else:
            ...
            if jobRating > 0 and keyword_index==0:
        ...
        i = 0
        #print("selfiee.matchedrows",self.matchedRows)
        if keyword_index==1:
            self.ratings=ratings_arr
            self.matchedRows=arr
            self.totalMatchedRows=tr
            #print(self.matchedRows)
            #print(self.ratings)
        while i < self.totalMatchedRows :
            self.sortedOrder.append( i )
            i += 1
        #print("sortedOrderOut",self.sortedOrder)
    ...
    def print_results (self):
        ...
        while (current_page_number-1)*page_size+i < self.totalMatchedRows and i < page_size  :
            ...
            col3 = "<a href=\'/~ckw91/cgi-bin/getjob.pl?id="+row[0]+"&job_title="+row[2]+ \
        ...
        if ( current_page_number > 1 ) :
            ...
            print ("<a href=\'/~ckw91/cgi-bin/jobsearch.py?"+self.q_str+'&requested_page_number='+str(prepage)+"\'> < </a>")
        else :
            print (" ")
        ...
        if total_page > 1 :
            ...
            while i < total_page :
                ...
                if page_number == current_page_number :
                    ...
                    print ("<a href=\'/~ckw91/cgi-bin/jobsearch.py?"+new_query_string+"\'>"+str(page_number)+"</a>")
                print (" ")
                i += 1

        if current_page_number < total_page :
            next_page = current_page_number + 1
            print ("<a href=\'/~ckw91/cgi-bin/jobsearch.py?"+self.q_str+'&requested_page_number='+str(next_page)+"\'> > </a>")
        elif total_page > 1 :
            print (" >")
